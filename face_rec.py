...
def recognize_query_image(query_img_path, train_images, train_labels, mean_face, eigenvectors, threshold, label_names, image_size=(100, 100)):
    ...
    # Print the results
    print(f"Predicted label: {predicted_label}, Name: {predicted_name}, Distance: {min_dist}")
...
image = cv2.imread("orl_faces_test/s21/8.pgm", cv2.IMREAD_GRAYSCALE)  # <-- Change this to your test image file

# Call the function to recognize the query image.
recognize_query_image("orl_faces_test/s20/8.pgm", train_images, train_labels, mean_face, eigenvectors, threshold, train_label_names, image_size=(100, 100))
...
# def recognize_face(image,pc_num, threshold = 0):
#     train_images, train_labels, label_names = read_images("orl_faces_train", image_size=(100, 100), detect=True)
#     # test_images, test_labels, test_label_names = read_images("orl_faces_test", image_size=(100, 100), detect=True)
#     mean_face, eigenvalues, eigenvectors = compute_pca(train_images, pc_num)

#     image = cv2.resize(image, (100, 100))

#     # Compute distances to all training faces
#     train_projections = [project_face(train_face, mean_face, eigenvectors) for train_face in train_images]
#     proj_test = project_face(image.flatten(), mean_face, eigenvectors)
#     dists = [euclidean_distance(proj_test, train_proj) for train_proj in train_projections]
#     min_index = np.argmin(dists)
#     min_dist = dists[min_index]
...
# # Example usage:
# image = cv2.imread("orl_faces_test/s21/8.pgm", cv2.IMREAD_GRAYSCALE)
# predicted_label, subject_name, min_dist= recognize_face(image, pc_num=100, threshold=5000)
# print(f"Predicted label: {predicted_label}, Subject name: {subject_name}, Distance: {min_dist}")
